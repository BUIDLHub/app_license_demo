...
contract LicenseMain is Ownable, LicenseManager {
    ...
    * @dev register a new vendor with the contract. Throws if address already registered as vendor.
     */
    function registerVendor(string memory _name) public proxyOnly {
     ...
     */
    function registerProduct(string memory _name) public proxyOnly {
    ...
    function registerLicenseSpecs(uint256 _productID,
                                  ...
                                  string memory _name,
                                  uint256 _price,
                                  uint256 _duration) public proxyOnly {
     ...
     */
    function vendorName(uint256 _vendorId) public proxyOnly view returns(string memory) {
    ...
    function productInfo(uint256 _prodID)
            public proxyOnly view returns(uint256, uint256, uint256, string memory) {
    ...
    */
    function licenseInfo(uint256 _licenseID)
            public proxyOnly view returns(uint256, uint256, uint256, uint256) {
